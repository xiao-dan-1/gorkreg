...
from config import (
    ACCOUNT_TYPE_CLI_PROXY,
    ACCOUNT_TYPE_CONSOLE_SSO,
    ACCOUNT_TIER_BASIC,
    ACCOUNT_TIER_SUPER,
    AUTH_FILE,
    CLI_VERSION,
...
class GrokCredentials:
    ...
    # Account type: cli_proxy (OIDC session → cli-chat-proxy) or console_sso
    account_type: str = ACCOUNT_TYPE_CLI_PROXY
    # Account tier: basic / super / heavy
    tier: str = ACCOUNT_TIER_SUPER
...
def _entry_to_creds(name: str, entry: dict[str, Any]) -> GrokCredentials:
    ...
    # Determine account type from entry metadata
    account_type = entry.get("account_type") or ACCOUNT_TYPE_CLI_PROXY
    # Determine tier: explicit > inferred from type > default super
    tier = entry.get("tier") or ""
    if not tier:
        if account_type == ACCOUNT_TYPE_CONSOLE_SSO:
            tier = ACCOUNT_TIER_BASIC
        else:
            tier = ACCOUNT_TIER_SUPER
...
def upstream_headers(token: str, model: str, *, account_type: str = None) -> dict[str, str]:
    """Headers required by upstream (cli-chat-proxy or console.x.ai).

    For console_sso accounts, the token is a cookie-based SSO credential
    sent differently from OIDC Bearer tokens.
    """
    if account_type is None:
        account_type = ACCOUNT_TYPE_CLI_PROXY
    if account_type == ACCOUNT_TYPE_CONSOLE_SSO:
        # Console SSO: token is the SSO cookie value
        return {
    ...
    # Default: cli-chat-proxy with Bearer token
    return {
        "Content-Type": "application/json",
...
def upstream_url_for(creds: GrokCredentials | None, model: str) -> str:
    """Return the correct upstream URL for the account type and model."""
    from config import CONSOLE_MODELS
    if creds and creds.account_type == ACCOUNT_TYPE_CONSOLE_SSO:
        return f"{CONSOLE_BASE_URL}/chat/completions"
    if model in CONSOLE_MODELS:
        return f"{CONSOLE_BASE_URL}/chat/completions"
